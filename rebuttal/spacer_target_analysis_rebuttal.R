...
MGEs_with_AMR <- read_csv("final_dfs/MGEs_with_AMR.csv") %>%
  filter(pident > 90) %>% 
...
spacer_targets <- read_csv("finaL_dfs/spacer_targets.csv") 
...
spacers_against_MGEs_w_AMR_convert_binom <- spacers_tidy %>% 
  ...
  filter(pident == 100) %>% # get spacers only with > 95% matches
...
# spacers per target
virus <- spacers_against_MGEs_w_AMR_convert_binom %>%
  filter(target == "virus")

other <- spacers_against_MGEs_w_AMR_convert_binom %>%
  filter(target != "virus")
...
ggsave(plot=combined_plot, "rebuttal_spacer_type_models.pdf", width = 25, dpi = 300, height = 20, units = "cm")

# write model output data frames

write.csv(summary_w_amr$solutions, file = "rebuttal_targets_w_amr.csv")
write.csv(summary_wo_amr$solutions, file = "rebuttal_targets_wo_amr.csv")